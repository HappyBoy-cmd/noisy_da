...
sys.path.insert(0, "/home/dyt/Messi_du/2022/RDA")
...
### analysis
#==============eval
def save_loss(model_instance, input_loader, iter_num):
    ori_train_state = model_instance.is_train
    model_instance.set_train(False)
    num_iter = len(input_loader)
    iter_test = iter(input_loader)
    ...
    # return {'accuracy':accuracy}, all_feature
    # save
    np.savez(os.path.join('/home/dyt/Messi_du/2022/RDA/plot', 'TCL_loss_' + str(iter_num) + '_plot'), all_classifier_loss.cpu().numpy(), all_labels.cpu().numpy(), all_labels_real.cpu().numpy())


##
def train(model_instance, train_source_clean_loader, train_source_noisy_loader,
          train_target_loader, test_target_loader, group_ratios, max_iter,
          optimizer, lr_scheduler, eval_interval):
    ...
    while True:
        for (datas_clean, datas_noisy,
             datat) in tqdm.tqdm(zip(train_source_clean_loader,
                                     train_source_noisy_loader,
                                     train_target_loader),
                                 total=min(len(train_source_clean_loader),
            ...
            if model_instance.use_gpu:
                inputs_source, inputs_source_noisy, inputs_target, labels_source = Variable(
                    inputs_source).cuda(), Variable(inputs_source_noisy).cuda(
                    ), Variable(inputs_target).cuda(), Variable(
                        labels_source).cuda(), 
            else:
                inputs_source, inputs_source_noisy, inputs_target, labels_source = Variable(
                    inputs_source), Variable(inputs_source_noisy), Variable(
            ...
            if iter_num % eval_interval == 0 and iter_num != 0:
                eval_result, all_feature = evaluate(model_instance,
                                                    test_target_loader)
                save_loss(model_instance, train_source_clean_loader, iter_num)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config',
                        ...
                        default='/home/dyt/Messi_du/2022/RDA/config/dann.yml')
    ...
    parser.add_argument(
        '--src_address',
        default=
        "/home/dyt/Messi_du/2022/RDA/data/Office-31/amazon_uniform_noisy_0.4.txt",
        type=str,
        help='address of image list of source dataset')
    parser.add_argument(
        '--tgt_address',
        default="/home/dyt/Messi_du/2022/RDA/data/Office-31/webcam.txt",
        type=str,
        help='address of image list of target dataset')
    parser.add_argument(
        '--stats_file',
        default=
        "/home/dyt/Messi_du/2022/RDA/statistic/TCL-A2W-uniform-noisy-0.4-2022-02-22-13-37-40.pkl",
        type=str,
    ...
    model_instance = TCL(base_net='ResNet50',
                         width=width,
                         use_gpu=True,
                         class_num=class_num,
                         srcweight=srcweight)

    train_source_clean_loader = load_images_TCL(source_file,
                                                batch_size=32,
                                                is_cen=is_cen,
                                                split_noisy=False)
